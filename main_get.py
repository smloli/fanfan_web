...
class Server():
    ...
    def _recv(http_server):
        ...
        if path == '/':
            path = '/download.html'
        Server._return_html(path,http_server,recv_content)

    @staticmethod
    def _return_html(path,http_server,recv_content):
        if recv_content[:6] == 'GET /?':
            url_info_index_left = recv_content.find('https')
            url_info_index_right = recv_content.find(' HTTP')
            url = recv_content[url_info_index_left:url_info_index_right]
    ...
    def run(self):
        while True:
            ...
            loli = threading.Thread(target=Server._recv, args=(http_server,))
            loli.setDaemon(True)
            loli.start()
