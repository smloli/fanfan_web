...
class Novel:
    ...
    def get_xpath(url):
        ...
        if not url_cid:
            return None
        # 小说名
        novelname = html.xpath("//title/text()")[0]
        name_reg = r'》(.*?)作'
        replace_content = re.findall(name_reg, novelname)[0]
        novelname = novelname.replace(replace_content, '')
        novelname = re.sub(r'[/|:|【完结】\-饭饭txt论坛聚集百万书友]', '', novelname)
        root_path = '/home/novel/'
        # 判断Novel文件夹是否存在，不存在就创建
        if not os.path.exists(root_path):
            os.mkdir(root_path)
        # 下载路径
        path = root_path + novelname + '.txt'
        # 判断文件是否存在
